#' Locally Breusch-Pagan Lagrange Multiplier Test Based on GWPR
#'
#' @description This function perform Breusch-Pagan Lagrange Multiplier test in each regression based on different subsamples
#'
#' @usage GWPR.plmtest(formula, data, index, SDF, bw = NULL, adaptive = FALSE, p = 2,
...
#' bw.AIC.Fix <- 2.010529
#'
#' GWPR.plmtest.resu.F <- GWPR.plmtest(formula = formula.GWPR, data = TransAirPolCalif,
...
GWPR.plmtest <- function(formula, data, index, SDF, bw = NULL, adaptive = FALSE, p = 2,
                         kernel = "bisquare", longlat = FALSE)
{
  if(length(index) != 2)
  {
  ...
  if (adaptive)
  {
    result <- gwpr_A_plmtest(bw = bw, data = lvl1_data, SDF = SDF, index = index, ID_list = ID_num,
  ...
  {
    result <- gwpr_F_plmtest(bw = bw, data = lvl1_data, SDF = SDF, index = index, ID_list = ID_num,
                             formula = formula, p = p, longlat = longlat, adaptive = adaptive,
                             kernel = kernel, huge_data_size = huge_data_size)
  }
  return(result)
